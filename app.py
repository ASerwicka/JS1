...
class Gui(Tk):
    def __init__(self):
        ...
        self.chosen_dependency = DEPENDENCY
        self.chosen_education = EDUCATION
        self.combo_dep = None
        self.combo_edu = None
        ...
        self.up_left_frame = LabelFrame(self.upper_frame, bg="green", height=400, width=250)
        self.up_left_frame.pack(fill=BOTH, side=LEFT)
        self.up_left_frame.pack_propagate(False)

        self.up_right_frame = LabelFrame(self.upper_frame, bg="blue", height=400, width=650)
    ...
    def create_combobox_education(self):
        self.combo_edu = ttk.Combobox(self.up_left_frame, values=self.labels)
        self.combo_edu.set("Pick an education")
        self.combo_edu.pack(fill=X)

        self.combo_dep = ttk.Combobox(self.up_left_frame, values=('Wiek', 'Płeć'))
    ...
    def get_data_to_display(self):
        if self.chosen_dependency.__eq__('Ogólnie') and self.chosen_education.__eq__('Wszystkie'):
